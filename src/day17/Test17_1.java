...
public class Test17_1 implements Runnable {
	...
	public static class Rock {
		...
		public boolean canMove(HashSet<Point> occupied, Direction direction) {
			Point p;
	...
	public void run() {
		...
		for (spawnedCount = 0; spawnedCount < numToFall; spawnedCount++) {
			...
			int oldTallestRock = tallestRock;
			tallestRock = rock.getMaxHeight();
			for (int ii = oldTallestRock; ii <= tallestRock; ii++) {
				occupiedPositions.add(new Point(-1, ii));
				occupiedPositions.add(new Point(7, ii));
			}
		...
		System.out.println(tallestRock);
	}

	private void print(Rock rock, int maxHeight) {
		String str="";
		for(int iy=maxHeight; iy>=0; iy--) {
			str+="\n█";//░
